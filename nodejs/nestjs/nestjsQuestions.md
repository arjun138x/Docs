...
60. ### How does NestJS support Server-Sent Events (SSE), and what are the primary advantages of using SSE for real-time communication in web applications?
    ...
    Great! Since you're using **NestJS**, which is built with **TypeScript**, you can absolutely use both `extends` and `implements`. Here's how they differ in the NestJS/TypeScript context:
...
Use `extends` when one class **inherits behavior and properties** from another class.
...
In NestJS, you might use this to share logic across services or controllers.

---

## 🔹 `implements` — Interface Implementation

Use `implements` to enforce a class to **follow a structure** defined by an interface.
...
If you miss the `speak` method, TypeScript will give a **compile-time error**.

---

## 💡 NestJS Example (Real-world Style)

### Interface:

```typescript
export interface OnInitService {
  onInit(): void;
}
```
...
```typescript
@Injectable()
export class MyService implements OnInitService {
  onInit() {
...
export class AdvancedService extends BaseService {
  runTask() {
    this.log("Running advanced task");
  }
}
```
...
| Keyword      | Purpose                                 | Example Use in NestJS                             |
| ------------ | --------------------------------------- | ------------------------------------------------- |
| `extends`    | Inherit methods/properties from a class | Base services or abstract class reuse             |
| `implements` | Enforce structure from an interface     | Define service/controller contracts or lifecycles |

Let me know if you want examples related to NestJS lifecycles like `OnModuleInit`, which use interfaces too.
